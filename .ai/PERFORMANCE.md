...
- **DONE — generated-registry/Emoji/JWK/OpenAPI/PKCS#12 budget:** the completed 131-route artifact is 646 files / 18,678,871 B raw / 5,432,106 B gzip. Relative to the 129-route XLSX checkpoint, shell including document moves only +1,366 B raw and -22 B gzip to 845,960/247,436 B; mandatory Workbox moves by the same amount to 9 files / 899,808/297,798 B and retains 100,192 B raw headroom. The generator-only/Emoji checkpoint had already reduced the shell asset closure to 843,057/246,937 B, so no additional shell/PWA optimization was justified by measurement. JWK/JWKS verification stays inside the existing lazy JWT route, whose own chunk is 16,890/6,010 B. OpenAPI adds 115,352/38,521 B and owns a 101,774/32,190 B worker under 130/45 and 110/36 kB ceilings. PKCS#12 adds 417,172/95,332 B and owns a 399,673/87,264 B PKIjs worker under 450/105 and 420/95 kB ceilings; no new artifact exceeds the global 500 kB limit and neither route enters Workbox. The clean source-dev run prebundles PKIjs explicitly, passes 4/4 feature scenarios without an optimizer reload, and traverses all 131 routes in 5.0 minutes. The current build transforms 24,176 modules.
...
- **DONE — Emoji windowed rendering/search slice:** the paged 60-card presentation has been replaced by true responsive row windowing with one-row overscan, stable total geometry, group headers, ARIA position/set metadata, and arrow/Home/End navigation across items that are not mounted. Query-triggered Fuse indexing/search runs in a bounded terminate-and-replace worker with explicit cancellation, stale-result, timeout, and disposal guards. Repeated production 4x-CPU runs stay below the strict `<50 ms` Long Task gate; the 400,601 B raw / 68,523 B gzip worker remains route-local and demand-loaded.
...
**Current Emoji result (2026-08-16):** one responsive row window renders only
the visible slice plus one overscan row while preserving full scroll geometry,
headers, ARIA position/set metadata, and keyboard navigation across unmounted
items. Search builds and queries Fuse inside a bounded route-owned worker;
replacement, explicit cancellation, stale-result, timeout, and disposal paths
are covered. Repeated production 4x-CPU flows remain below the strict `<50 ms`
Long Task gate.
...
**Current result (2026-08-16):** true row windowing has replaced pagination for
both catalog and search results. Copy controls are keyboard-native, complete
ZWJ/flag sequences are retained, group headers remain represented, and
secondary keyword metadata loads only on search. Fuse execution is cancellable
and worker-backed; repeated 4x-CPU production runs pass the strict `<50 ms`
Long Task gate.
...
| Emoji bounded rendering | 14,396 nodes and 1.06 s route task | **Achieved:** one responsive row-windowed grid preserves total geometry, groups, Fuse relevance, ARIA position, and keyboard navigation while mounting only the visible slice plus one overscan row; bounded worker search repeatedly passes the strict 4x-CPU `<50 ms` Long Task gate |
