...
describe('Docker converter worker protocol', () => {
  it('accepts only exact non-empty tasks within the input bound', () => {
    expect(parseDockerConverterTask({ source: 'docker run nginx' })).toEqual({ source: 'docker run nginx' });
    expect(() => parseDockerConverterTask({ source: '', extra: true })).toThrow('Enter a Docker run command');
    expect(() => parseDockerConverterTask({ source: 'a'.repeat(DOCKER_CONVERTER_MAX_INPUT_BYTES + 1) }))
      .toThrow('limited');
  });
