...
describe('DockerConverterWorkerClient', () => {
  it('decodes a strict result and terminates the worker', async () => {
    ...
    const pending = client.run({ source: 'docker run nginx' });
    workers[0].emit({
      jobId: 1,
      type: 'result',
      result: {
  ...
  it('physically terminates replacement, cancellation, and timeout', async () => {
    ...
    const first = replaced.client.run({ source: 'docker run nginx' });
    const second = replaced.client.run({ source: 'docker run alpine' });
    ...
    const pending = timed.client.run({ source: 'docker run nginx' });
